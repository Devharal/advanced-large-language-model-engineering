...
class RoPEMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        **_,
        ...
        # Always return attn_weights — callers that don't need them ignore them.
...
def p1_verify_relative_position_property(head_dim: int = 8, base: float = 10_000.0):
    ...
    ─────────────────────────────
    apply_rotary_pos_emb() rotates BOTH q and k by the SAME position_ids.
    To rotate q at position m and k at position n INDEPENDENTLY we must
    call the function twice — once for q, once for k — each time passing
    a dummy tensor for the argument we don't want to use.
    """
    print("\n[P1] Verifying relative-position property of RoPE dot product …")
    cos, sin = precompute_rope_freqs(head_dim, max_seq_len=300, base=base)

    torch.manual_seed(0)
    q_raw = torch.randn(head_dim)
    k_raw = torch.randn(head_dim)

    q_4d = q_raw.view(1, 1, 1, head_dim)   # (1,1,1,d)
    k_4d = k_raw.view(1, 1, 1, head_dim)

    def dot_at(pos_q: int, pos_k: int) -> float:
        """Compute dot(rotate(q, pos_q), rotate(k, pos_k))."""
        # Rotate q at pos_q (k_4d used as placeholder for 2nd arg, unused)
        q_rot, _ = apply_rotary_pos_emb(
            q_4d, q_4d, cos, sin, position_ids=torch.tensor([pos_q]))
        # Rotate k at pos_k
        _, k_rot = apply_rotary_pos_emb(
            k_4d, k_4d, cos, sin, position_ids=torch.tensor([pos_k]))
        return (q_rot * k_rot).sum().item()

    results = []
    shift   = 50   # arbitrary translation applied to both m and n
    for m, n in [(5, 3), (50, 48), (100, 98), (10, 3), (20, 13), (75, 68)]:
        delta      = m - n
        dot_mn     = dot_at(m, n)
        dot_shifted = dot_at(m + shift, n + shift)   # same relative offset
        error      = abs(dot_mn - dot_shifted)
        results.append((m, n, delta, dot_mn, dot_shifted, error))
        print(f"   m={m:3d}, n={n:3d} | rel={delta:+3d} | "
              f"dot(q_m, k_n)={dot_mn:+.6f}  "
              f"dot(q_{{m+{shift}}}, k_{{n+{shift}}})={dot_shifted:+.6f}  "
              f"error={error:.2e}")
